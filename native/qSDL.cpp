...
// Event-driven dispatch (q_seteventcb): when set, the named unary q function
// is applied once per pump that saw relevant SDL activity (key/mouse/text,
// window expose/resize/focus, quit). The call happens inside the sd1
// callback on q's main thread - q only services sd1 when its event loop is
// idle, so this can never interrupt a running q computation.
static char g_eventcb[128] = {0}; // "" = disabled
...
static K on_pipe_readable(I fd) {
  ...
  while (SDL_PollEvent(&event)) {
    if (event.type == SDL_EVENT_QUIT) {
      ...
      wake = true;
    } else if (event.type == SDL_EVENT_KEY_DOWN || // repeats included - they
               event.type == SDL_EVENT_KEY_UP ||   // drive q-side key-hold
  ...
  // One coalesced callback per pump, however many events arrived above.
  if (wake && g_eventcb[0]) {
    K r = k(0, g_eventcb, ki(0), (K)0);
    if (r) {
      if (r->t == -128)
        fprintf(stderr, "[qSDL] event callback '%s' error: %s\n", g_eventcb,
                r->s ? r->s : "");
      r0(r);
    }
  }
...
K q_init(K w, K h, K s) {
  ...
  if (g_window) {
    ...
    // Aspect first: SDL_SetWindowSize clamps to the current aspect
    // constraint, so sizing before updating it leaves the window stuck at
    // the previous app's aspect ratio.
...
K q_shutdown(K unused) {
  ...
  g_quit = false;
  g_textbuf.clear();
  g_eventcb[0] = 0; // no window -> nothing to dispatch for
...
// ---------------------------------------------------------------------------
// q_seteventcb[name]
// name - char vector naming a unary q function (e.g. ".qos.FRAMETS"); "" to
// disable. While set, the event pump applies it (with a dummy 0i argument)
// on the q main thread whenever input/window events arrive, coalesced to one
// call per ~16ms pump. This is the push complement to polling from .z.ts:
// input renders the moment it happens and \t is left for animation only.
// Cleared by q_shutdown - re-register after the next q_init.
// ---------------------------------------------------------------------------
K q_seteventcb(K name) {
  if (name->t != KC)
    return krr((S) "type");
  if (name->n >= (J)sizeof(g_eventcb))
    return krr((S) "name too long");
  memcpy(g_eventcb, kC(name), name->n);
  g_eventcb[name->n] = 0;
  return (K)0;
}
